/**
 * @name Negative of Suspicious unused loop iteration variable
...
where
  ...
  (
   f.getAStmt().contains(v.getAnAccess()) or
   points_to_call_to_range(f.getIter().getAFlowNode()) or
   points_to_call_to_range(get_comp_iterable(f)) or
   name_acceptable_for_unused_variable(v) or
   f.getScope().getName() = "genexpr" or
   empty_loop(f) or
   one_item_only(f) or
   counting_loop(f) or
   implicit_repeat(f) )
select f, "loop variable " + v.getId()
